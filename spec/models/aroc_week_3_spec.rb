...
# ----------------------
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    names = Order.joins(order_items: :item).where("order_items.order_id = #{@order_12.id}").pluck("items.name")
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    items_for_user_3_third_order = Item.joins(order_items: :order).where("order_items.order_id = ? AND orders.user_id = ?", @order_9.id, @user_3.id).pluck(:name)
    # ------------------------------------------------------------
